...
public class UsersService {
	...
	public UserSearchResults searchUsers(UserQuery query) {
		Supplier<Stream<User>> sup = () -> USERS.stream().filter(
				(u) -> u.getFirstName().startsWith(query.getQuery())
						|| u.getLastName().startsWith(query.getQuery()));
		UserSearchResults results = new UserSearchResults();
		results.setTotalCount((int) sup.get().count());
		Stream<User> userStream = sup.get().sorted(comparator(query))
	...
	private Comparator<User> baseComparator(UserQuery query) {
		if ("last_name".equals(query.getSortBy()))
			return (User u1, User u2) -> u1.getLastName().compareTo(
					u2.getLastName());
	...
	public Optional<User> findByLoginAndPassword(String login,
		...
		return USERS.stream().filter((u) -> u.sameCredentials(login, password))
				.findFirst();
	}
	...
	public User findByLogin(String userLogin) {
		return USERS.stream().filter((u) -> u.getLogin().equals(userLogin))
				.findFirst().get();
	}
